...
import com.carl.mvpdemo.util.sql.entity.OrderInfo;
...
import com.carl.mvpdemo.util.sql.helper.DaoMasterUtils;
...
public class MainActivity extends BaseListActivity<MainI, MainPresenter> implements MainI {
    ...
    protected void initData() {
        mTitles.add("save data baseInfo");
        mTitles.add("save data orderInfo");
    }

    @Override
    protected void initView() {
        mToolbarManager.hideBackIcon();
        mToolbarManager.setToolbarTitle("GreenDao Demo");
        ...
        mBaseAdapter.setOnItemClickListener(new CommonBaseAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                switch (position) {
                    case 0:
                        BaseInfoDao baseInfoDao = DaoMasterUtils.getInstance().getDaoSection().getBaseInfoDao();
                        BaseInfo baseInfo = new BaseInfo(null, "123", "carl");
                        baseInfoDao.save(baseInfo);
                        ToastUtils.showLong("save success");
                        break;
                    case 1:
                        OrderInfoDao orderInfoDao = DaoMasterUtils.getInstance().getDaoSection().getOrderInfoDao();
                        OrderInfo orderInfo = new OrderInfo(null, "123456", "123");
